...
def log_error(msg):
    ...
    try:
        try:
            doc_dir = os.path.join(os.path.expanduser('~'), 'Documents', 'Assetto Corsa', 'cfg', 'apps')
            if not os.path.exists(doc_dir):
                os.makedirs(doc_dir, exist_ok=True)
            path = os.path.join(doc_dir, 'AlaschgariHUD_error.log')
        except:
        ...
        if lblDebugError != 0:
            ...
            ac.setText(lblDebugError, "Logs: ERR: " + last_line[:65])
    except:
        pass
...
# Color Palettes
BG_COLORS = [
    [0.008, 0.016, 0.07], # 0: Cyber Navy (Default)
...
TEXT_COLORS = [
    [0.0, 0.96, 1.0, 1.0],  # 0: Neon Cyan
    [1.0, 0.0, 0.4, 1.0],   # 1: Hot Pink
...
def getConfigPath():
    try:
        ...
        if not os.path.exists(doc_dir):
            os.makedirs(doc_dir, exist_ok=True)
        return os.path.join(doc_dir, 'AlaschgariHUD.ini')
    except:
...
def updateScaleTires(s):
    ...
    scale_tires = s
    ac.setSize(appTires, int(round(460 * scale_tires)), int(round(140 * scale_tires)))
    ac.setPosition(lblPressFL, int(round(15 * scale_tires)), int(round(25 * scale_tires)))
    ac.setFontSize(lblPressFL, int(round(24 * scale_tires)))
    ac.setPosition(lblPressRL, int(round(15 * scale_tires)), int(round(80 * scale_tires)))
    ac.setFontSize(lblPressRL, int(round(24 * scale_tires)))
    ac.setPosition(lblPressFR, int(round(445 * scale_tires)), int(round(25 * scale_tires)))
    ac.setFontSize(lblPressFR, int(round(24 * scale_tires)))
    ac.setPosition(lblPressRR, int(round(445 * scale_tires)), int(round(80 * scale_tires)))
    ac.setFontSize(lblPressRR, int(round(24 * scale_tires)))
    ac.setPosition(lblBrakeF, int(round(230 * scale_tires)), int(round(25 * scale_tires)))
    ac.setFontSize(lblBrakeF, int(round(24 * scale_tires)))
    ac.setPosition(lblBrakeR, int(round(230 * scale_tires)), int(round(80 * scale_tires)))
    ac.setFontSize(lblBrakeR, int(round(24 * scale_tires)))
    if imgChassis != 0:
        ac.setPosition(imgChassis, int(round(175 * scale_tires)), int(round(15 * scale_tires)))
        ac.setSize(imgChassis, int(round(110 * scale_tires)), int(round(110 * scale_tires)))
    if btnScaleTires != 0:
        ac.setPosition(btnScaleTires, 0, 0)
...
def updateScaleSpeed(s):
    ...
    global appSpeed, lblSpeed, lblSpeedLabel, imgSpeed
    scale_speed = s
    ac.setSize(appSpeed, int(round(380 * scale_speed)), int(round(220 * scale_speed)))
    if imgSpeed != 0:
        ac.setPosition(imgSpeed, int(round(15 * scale_speed)), int(round(14 * scale_speed)))
        ac.setSize(imgSpeed, int(round(192 * scale_speed)), int(round(192 * scale_speed)))
    ac.setPosition(lblSpeed, int(round(220 * scale_speed)), int(round(70 * scale_speed)))
    ac.setFontSize(lblSpeed, int(round(24 * scale_speed)))
    ac.setFontAlignment(lblSpeed, "left")
    ac.setPosition(lblSpeedLabel, int(round(220 * scale_speed)), int(round(130 * scale_speed)))
    ac.setFontSize(lblSpeedLabel, int(round(24 * scale_speed)))
    ac.setFontAlignment(lblSpeedLabel, "left")
    if btnScaleSpeed != 0:
        ac.setPosition(btnScaleSpeed, 0, 0)
...
def updateScaleGear(s):
    ...
    global appGear, lblGear, lblGForce, lblGearLabel, imgGearIcon
    scale_gear = s
    ac.setSize(appGear, int(round(380 * scale_gear)), int(round(220 * scale_gear)))
    if imgGearIcon != 0:
        ac.setPosition(imgGearIcon, int(round(15 * scale_gear)), int(round(14 * scale_gear)))
        ac.setSize(imgGearIcon, int(round(192 * scale_gear)), int(round(192 * scale_gear)))
    ac.setPosition(lblGear, int(round(220 * scale_gear)), int(round(50 * scale_gear)))
    ac.setFontSize(lblGear, int(round(24 * scale_gear)))
    ac.setFontAlignment(lblGear, "left")
    ac.setPosition(lblGForce, int(round(220 * scale_gear)), int(round(105 * scale_gear)))
    ac.setFontSize(lblGForce, int(round(24 * scale_gear)))
    ac.setFontAlignment(lblGForce, "left")
    ac.setPosition(lblGearLabel, int(round(220 * scale_gear)), int(round(150 * scale_gear)))
    ac.setFontSize(lblGearLabel, int(round(24 * scale_gear)))
    ac.setFontAlignment(lblGearLabel, "left")
    if btnScaleGear != 0:
        ac.setPosition(btnScaleGear, 0, 0)
...
def updateScalePedals(s):
    ...
    if imgPedalBrake != 0:
        ac.setPosition(imgPedalBrake, int(round(25 * scale_pedals)), int(round(34 * scale_pedals)))
        ac.setSize(imgPedalBrake, int(round(18 * scale_pedals)), int(round(18 * scale_pedals)))
    if imgPedalThrottle != 0:
        ac.setPosition(imgPedalThrottle, int(round(25 * scale_pedals)), int(round(56 * scale_pedals)))
        ac.setSize(imgPedalThrottle, int(round(18 * scale_pedals)), int(round(18 * scale_pedals)))

    ac.setPosition(lblPedalClutchVal, int(round(220 * scale_pedals)), int(round(10 * scale_pedals)))
    ac.setFontSize(lblPedalClutchVal, int(round(24 * scale_pedals)))
    ac.setPosition(lblPedalBrakeVal, int(round(220 * scale_pedals)), int(round(32 * scale_pedals)))
    ac.setFontSize(lblPedalBrakeVal, int(round(24 * scale_pedals)))
    ac.setPosition(lblPedalThrottleVal, int(round(220 * scale_pedals)), int(round(54 * scale_pedals)))
    ac.setFontSize(lblPedalThrottleVal, int(round(24 * scale_pedals)))
    if btnScalePedals != 0:
        ac.setPosition(btnScalePedals, 0, 0)
...
def updateScaleKers(s):
    ...
    scale_kers = s
    ac.setSize(appKers, int(round(240 * scale_kers)), int(round(55 * scale_kers)))
    if imgKers != 0:
        ac.setPosition(imgKers, int(round(25 * scale_kers)), int(round(15 * scale_kers)))
        ac.setSize(imgKers, int(round(24 * scale_kers)), int(round(24 * scale_kers)))
    if imgWear != 0:
        ac.setPosition(imgWear, int(round(135 * scale_kers)), int(round(15 * scale_kers)))
        ac.setSize(imgWear, int(round(24 * scale_kers)), int(round(24 * scale_kers)))
    if btnScaleKers != 0:
        ac.setPosition(btnScaleKers, 0, 0)
...
def updateScaleTimes(s):
    ...
    global appTimes, lblTimesCurrent, lblTimesBest, lblTimesLast, lblTimesLaps, imgTimes
    scale_times = s
    ac.setSize(appTimes, int(round(500 * scale_times)), int(round(220 * scale_times)))
    if imgTimes != 0:
        ac.setPosition(imgTimes, int(round(15 * scale_times)), int(round(14 * scale_times)))
        ac.setSize(imgTimes, int(round(192 * scale_times)), int(round(192 * scale_times)))
    ac.setPosition(lblTimesBest, int(round(220 * scale_times)), int(round(35 * scale_times)))
    ac.setFontSize(lblTimesBest, int(round(24 * scale_times)))
    ac.setFontAlignment(lblTimesBest, "left")
    ac.setPosition(lblTimesLast, int(round(220 * scale_times)), int(round(75 * scale_times)))
    ac.setFontSize(lblTimesLast, int(round(24 * scale_times)))
    ac.setFontAlignment(lblTimesLast, "left")
    ac.setPosition(lblTimesCurrent, int(round(220 * scale_times)), int(round(115 * scale_times)))
    ac.setFontSize(lblTimesCurrent, int(round(24 * scale_times)))
    ac.setFontAlignment(lblTimesCurrent, "left")
    ac.setPosition(lblTimesLaps, int(round(220 * scale_times)), int(round(155 * scale_times)))
    ac.setFontSize(lblTimesLaps, int(round(24 * scale_times)))
    ac.setFontAlignment(lblTimesLaps, "left")
    if btnScaleTimes != 0:
        ac.setPosition(btnScaleTimes, 0, 0)
...
def updateScaleFuel(s):
    ...
    global appFuel, lblFuelCurrent, lblFuelCons, lblFuelEst, lblFuelTemps, imgFuel
    scale_fuel = s
    ac.setSize(appFuel, int(round(500 * scale_fuel)), int(round(220 * scale_fuel)))
    if imgFuel != 0:
        ac.setPosition(imgFuel, int(round(15 * scale_fuel)), int(round(14 * scale_fuel)))
        ac.setSize(imgFuel, int(round(192 * scale_fuel)), int(round(192 * scale_fuel)))
    ac.setPosition(lblFuelCons, int(round(220 * scale_fuel)), int(round(35 * scale_fuel)))
    ac.setFontSize(lblFuelCons, int(round(24 * scale_fuel)))
    ac.setFontAlignment(lblFuelCons, "left")
    ac.setPosition(lblFuelEst, int(round(220 * scale_fuel)), int(round(75 * scale_fuel)))
    ac.setFontSize(lblFuelEst, int(round(24 * scale_fuel)))
    ac.setFontAlignment(lblFuelEst, "left")
    ac.setPosition(lblFuelCurrent, int(round(220 * scale_fuel)), int(round(115 * scale_fuel)))
    ac.setFontSize(lblFuelCurrent, int(round(24 * scale_fuel)))
    ac.setFontAlignment(lblFuelCurrent, "left")
    ac.setPosition(lblFuelTemps, int(round(220 * scale_fuel)), int(round(155 * scale_fuel)))
    ac.setFontSize(lblFuelTemps, int(round(24 * scale_fuel)))
    ac.setFontAlignment(lblFuelTemps, "left")
    if btnScaleFuel != 0:
        ac.setPosition(btnScaleFuel, 0, 0)
...
def updateScalePerf(s):
    ...
    scale_perf = s
    ac.setSize(appPerf, int(round(260 * scale_perf)), int(round(80 * scale_perf)))
    if imgPerf != 0:
        ac.setPosition(imgPerf, int(round(25 * scale_perf)), int(round(28 * scale_perf)))
        ac.setSize(imgPerf, int(round(24 * scale_perf)), int(round(24 * scale_perf)))
    ac.setPosition(lblPerfDelta, int(round(130 * scale_perf)), int(round(38 * scale_perf)))
    ac.setFontSize(lblPerfDelta, int(round(24 * scale_perf)))
    ac.setPosition(lblPerfTurbo, int(round(25 * scale_perf)), int(round(10 * scale_perf)))
    ac.setFontSize(lblPerfTurbo, int(round(24 * scale_perf)))
    ac.setPosition(lblPerfDRS, int(round(235 * scale_perf)), int(round(10 * scale_perf)))
    ac.setFontSize(lblPerfDRS, int(round(24 * scale_perf)))
    if btnScalePerf != 0:
        ac.setPosition(btnScalePerf, 0, 0)
...
def updateScaleDamage(s):
    ...
    scale_damage = s
    ac.setSize(appDamage, int(round(260 * scale_damage)), int(round(80 * scale_damage)))
    if imgDamage != 0:
        ac.setPosition(imgDamage, int(round(25 * scale_damage)), int(round(28 * scale_damage)))
        ac.setSize(imgDamage, int(round(24 * scale_damage)), int(round(24 * scale_damage)))
    ac.setPosition(lblDamageEngine, int(round(130 * scale_damage)), int(round(38 * scale_damage)))
    ac.setFontSize(lblDamageEngine, int(round(24 * scale_damage)))
    ac.setPosition(lblDamageAero, int(round(25 * scale_damage)), int(round(10 * scale_damage)))
    ac.setFontSize(lblDamageAero, int(round(24 * scale_damage)))
    ac.setPosition(lblDamageTrans, int(round(235 * scale_damage)), int(round(10 * scale_damage)))
    ac.setFontSize(lblDamageTrans, int(round(24 * scale_damage)))
    if btnScaleDamage != 0:
        ac.setPosition(btnScaleDamage, 0, 0)
...
def updateScaleTrack(s):
    ...
    scale_track = s
    ac.setSize(appTrack, int(round(260 * scale_track)), int(round(80 * scale_track)))
    if imgTrack != 0:
        ac.setPosition(imgTrack, int(round(25 * scale_track)), int(round(28 * scale_track)))
        ac.setSize(imgTrack, int(round(24 * scale_track)), int(round(24 * scale_track)))
    ac.setPosition(lblTrackGrip, int(round(130 * scale_track)), int(round(38 * scale_track)))
    ac.setFontSize(lblTrackGrip, int(round(24 * scale_track)))
    ac.setPosition(lblTrackWind, int(round(25 * scale_track)), int(round(10 * scale_track)))
    ac.setFontSize(lblTrackWind, int(round(24 * scale_track)))
    if btnScaleTrack != 0:
        ac.setPosition(btnScaleTrack, 0, 0)
...
def updateScaleDebug(s):
    ...
    global appDebug, lblDebugError, lblDebugVersion, imgDebug
    scale_debug = s
    ac.setSize(appDebug, int(round(620 * scale_debug)), int(round(220 * scale_debug)))
    if imgDebug != 0:
        ac.setPosition(imgDebug, int(round(15 * scale_debug)), int(round(14 * scale_debug)))
        ac.setSize(imgDebug, int(round(192 * scale_debug)), int(round(192 * scale_debug)))
    ac.setPosition(lblDebugVersion, int(round(220 * scale_debug)), int(round(60 * scale_debug)))
    ac.setFontSize(lblDebugVersion, int(round(24 * scale_debug)))
    ac.setFontAlignment(lblDebugVersion, "left")
    ac.setPosition(lblDebugError, int(round(220 * scale_debug)), int(round(110 * scale_debug)))
    ac.setFontSize(lblDebugError, int(round(24 * scale_debug)))
    ac.setFontAlignment(lblDebugError, "left")
    if btnScaleDebug != 0:
        ac.setPosition(btnScaleDebug, 0, 0)
...
def createBoldLabel(app, text):
    ...
    try:
        ac.initFont("Segoe UI")
        ac.setFont(lbl, "Segoe UI")
    except:
        pass
    try:
...
def acMain(ac_version):
    global scale_shift, scale_tires, scale_speed, scale_gear, scale_pedals, scale_kers, scale_times, scale_fuel, scale_perf, scale_damage, scale_track, scale_debug
    global lblDebugError, lblDebugVersion, bg_color_idx, opacity_pct, text_color_idx
    ...
    global imgSpeed, imgGearIcon, imgTimes, imgFuel, imgDebug
    ...
    try:
        ...
        _, sliderScaleShift = addConfigRow("Scale: Shift Lights", 50, 400, 5, int(scale_shift * 100), y); y += 30
        _, sliderScaleTires = addConfigRow("Scale: Tires Status", 50, 400, 5, int(scale_tires * 100), y); y += 30
        _, sliderScaleSpeed = addConfigRow("Scale: Speedometer", 50, 400, 5, int(scale_speed * 100), y); y += 30
        _, sliderScaleGear = addConfigRow("Scale: Gear Box", 50, 400, 5, int(scale_gear * 100), y); y += 30
        _, sliderScalePedals = addConfigRow("Scale: Pedals Input", 50, 400, 5, int(scale_pedals * 100), y); y += 30
        _, sliderScaleKers = addConfigRow("Scale: KERS & Wear", 50, 400, 5, int(scale_kers * 100), y); y += 30
        _, sliderScaleTimes = addConfigRow("Scale: Laps & Times", 50, 400, 5, int(scale_times * 100), y); y += 30
        _, sliderScaleFuel = addConfigRow("Scale: Fuel Calc", 50, 400, 5, int(scale_fuel * 100), y); y += 30
        _, sliderScalePerf = addConfigRow("Scale: Performance", 50, 400, 5, int(scale_perf * 100), y); y += 30
        _, sliderScaleDamage = addConfigRow("Scale: Damages", 50, 400, 5, int(scale_damage * 100), y); y += 30
        _, sliderScaleTrack = addConfigRow("Scale: Track Wind", 50, 400, 5, int(scale_track * 100), y); y += 30
        _, sliderScaleDebug = addConfigRow("Scale: Debug Console", 50, 400, 5, int(scale_debug * 100), y)
        ...
        ac.setFontAlignment(lblBrakeF, "center")
        ...
        ac.setFontAlignment(lblBrakeR, "center")
        ...
        try:
            ...
            imgSpeed = ac.addLabel(appSpeed, "")
            ac.setBackgroundTexture(imgSpeed, "apps/python/AlaschgariHUD/images/speedometer_icon.png")
            ac.setPosition(imgSpeed, int(round(25 * scale_speed)), int(round(28 * scale_speed)))
            ac.setSize(imgSpeed, int(round(24 * scale_speed)), int(round(24 * scale_speed)))

            lblSpeed = createBoldLabel(appSpeed, "0 KM/H")
            ac.setPosition(lblSpeed, int(round(220 * scale_speed)), int(round(42 * scale_speed)))
            ac.setFontSize(lblSpeed, int(round(24 * scale_speed)))
            ac.setFontAlignment(lblSpeed, "left")

            lblSpeedLabel = createBoldLabel(appSpeed, "Speedometer")
            ac.setPosition(lblSpeedLabel, int(round(220 * scale_speed)), int(round(105 * scale_speed)))
            ac.setFontSize(lblSpeedLabel, int(round(24 * scale_speed)))
            ac.setFontAlignment(lblSpeedLabel, "left")
            ac.setFontColor(lblSpeedLabel, 0.4, 0.4, 0.4, 1.0)
        except Exception as e:
            log_error("Speedometer init failed: " + str(e))
        ...
        try:
            ...
            ac.addRenderCallback(appGear, drawGearGL)

            btnScaleGear = ac.addButton(appGear, "+")
            ac.setText(btnScaleGear, "+")
            ac.addOnClickedListener(btnScaleGear, onScaleGearClick)

            imgGearIcon = ac.addLabel(appGear, "")
            ac.setBackgroundTexture(imgGearIcon, "apps/python/AlaschgariHUD/images/gear_icon.png")
            ac.setPosition(imgGearIcon, int(round(25 * scale_gear)), int(round(28 * scale_gear)))
            ac.setSize(imgGearIcon, int(round(24 * scale_gear)), int(round(24 * scale_gear)))

            lblGear = createBoldLabel(appGear, "G1")
            ac.setPosition(lblGear, int(round(220 * scale_gear)), int(round(34 * scale_gear)))
            ac.setFontSize(lblGear, int(round(24 * scale_gear)))
            ac.setFontAlignment(lblGear, "left")

            lblGForce = createBoldLabel(appGear, "0.00 / 0.00")
            ac.setPosition(lblGForce, int(round(220 * scale_gear)), int(round(75 * scale_gear)))
            ac.setFontSize(lblGForce, int(round(24 * scale_gear)))
            ac.setFontAlignment(lblGForce, "left")

            lblGearLabel = createBoldLabel(appGear, "G-Force / Gear")
            ac.setPosition(lblGearLabel, int(round(220 * scale_gear)), int(round(105 * scale_gear)))
            ac.setFontSize(lblGearLabel, int(round(24 * scale_gear)))
            ac.setFontAlignment(lblGearLabel, "left")
            ac.setFontColor(lblGearLabel, 0.4, 0.4, 0.4, 1.0)
        except Exception as e:
            log_error("Gear/GForce init failed: " + str(e))
        ...
        try:
            ...
            log_error("Pedals init failed: " + str(e))
        ...
        try:
            ...
            log_error("KERS/Wear init failed: " + str(e))
        ...
        try:
            ...
            imgTimes = ac.addLabel(appTimes, "")
            ac.setBackgroundTexture(imgTimes, "apps/python/AlaschgariHUD/images/times_icon.png")
            ac.setPosition(imgTimes, int(round(25 * scale_times)), int(round(38 * scale_times)))
            ac.setSize(imgTimes, int(round(24 * scale_times)), int(round(24 * scale_times)))
            ...
            log_error("Times init failed: " + str(e))
        ...
        try:
            ...
            imgFuel = ac.addLabel(appFuel, "")
            ac.setBackgroundTexture(imgFuel, "apps/python/AlaschgariHUD/images/fuel_icon.png")
            ac.setPosition(imgFuel, int(round(25 * scale_fuel)), int(round(38 * scale_fuel)))
            ac.setSize(imgFuel, int(round(24 * scale_fuel)), int(round(24 * scale_fuel)))
            ...
            log_error("Fuel init failed: " + str(e))
        ...
        try:
            ...
            ac.addRenderCallback(appPerf, drawPerfGL)
            ...
            log_error("Performance init failed: " + str(e))
        ...
        try:
            ...
            ac.addRenderCallback(appDamage, drawDamageGL)
            ...
            log_error("Damage init failed: " + str(e))
        ...
        try:
            ...
            ac.addRenderCallback(appTrack, drawTrackGL)
            ...
            log_error("Track init failed: " + str(e))

        # ---------------------------------------------
        # 5g. APP: INFO & ERROR LOG (380px x 80px)
        # ---------------------------------------------
        try:
            appDebug = ac.newApp("AlaschgariHUD - Info & Logs")
            ac.setSize(appDebug, int(round(380 * scale_debug)), int(round(80 * scale_debug)))
            ac.setTitle(appDebug, "")
            ac.drawBorder(appDebug, 0)
            ac.setIconPosition(appDebug, -10000, -10000)
            ac.addRenderCallback(appDebug, drawDebugGL)

            btnScaleDebug = ac.addButton(appDebug, "+")
            ac.setText(btnScaleDebug, "+")
            ac.addOnClickedListener(btnScaleDebug, onScaleDebugClick)

            imgDebug = ac.addLabel(appDebug, "")
            ac.setBackgroundTexture(imgDebug, "apps/python/AlaschgariHUD/images/debug_icon.png")
            ac.setPosition(imgDebug, int(round(25 * scale_debug)), int(round(24 * scale_debug)))
            ac.setSize(imgDebug, int(round(32 * scale_debug)), int(round(32 * scale_debug)))

            lblDebugVersion = createBoldLabel(appDebug, "AlaschgariHUD v2.5.0 (Alpha)")
            ac.setPosition(lblDebugVersion, int(round(70 * scale_debug)), int(round(12 * scale_debug)))
            ac.setFontSize(lblDebugVersion, int(round(24 * scale_debug)))
            ac.setFontColor(lblDebugVersion, 0.0, 0.96, 1.0, 1.0) # Neon Cyan

            lblDebugError = createBoldLabel(appDebug, "Logs: System Health: OK")
            ac.setPosition(lblDebugError, int(round(70 * scale_debug)), int(round(44 * scale_debug)))
            ac.setFontSize(lblDebugError, int(round(24 * scale_debug)))
            ac.setFontColor(lblDebugError, 0.0, 1.0, 0.4, 1.0) # Toxic green for OK state
        except Exception as e:
            log_error("Debug init failed: " + str(e))
...
def getTireColor(temp):
    try:
        ...
        t = 0.0
    if t < 60.0:
        return [0.0, 0.4, 1.0, 0.9]  # Blue
    elif t < 75.0:
        return [0.0, 0.96, 1.0, 0.9] # Cyan
    elif t < 95.0:
        return [0.0, 1.0, 0.53, 0.9] # Green
    elif t < 110.0:
        return [1.0, 0.8, 0.0, 0.9]  # Yellow
    else:
        return [1.0, 0.0, 0.4, 0.9]  # Hot Pink / Red

def drawBorderAndCornersGL(app, w, h, scale):
    global text_color_idx
    try:
        c = TEXT_COLORS[text_color_idx]
        w_int = int(round(w))
        h_int = int(round(h))
        ...
        # Border
        ac.glColor4f(c[0], c[1], c[2], 0.15)
        ac.glQuad(0, 0, w_int, 1)
        ac.glQuad(0, h_int - 1, w_int, 1)
        ac.glQuad(0, 0, 1, h_int)
        ac.glQuad(w_int - 1, 0, 1, h_int)
        
        # Corner Accents
        ac.glColor4f(c[0], c[1], c[2], 0.85)
        sz = int(round(8 * scale))
        th = int(round(2 * scale))
        if th < 1: th = 1
        if sz < 4: sz = 4
        ...
        ac.glQuad(0, 0, th, sz)
        # Top-Right
        ac.glQuad(w_int - sz, 0, sz, th)
        ac.glQuad(w_int - th, 0, th, sz)
        # Bottom-Left
        ac.glQuad(0, h_int - th, sz, th)
        ac.glQuad(0, h_int - sz, th, sz)
        # Bottom-Right
        ac.glQuad(w_int - sz, h_int - th, sz, th)
        ac.glQuad(w_int - th, h_int - sz, th, sz)
    except:
        pass
...
def drawShiftGL(deltaT):
    global rpms, maxRpm, scale_shift, show_rpm, appShift
    if not show_rpm or maxRpm <= 0:
        return

    try:
        # Render panel background, border, and corners
        ac.glColor4f(0.008, 0.016, 0.07, 0.88)
        ac.glQuad(0, 0, int(round(480 * scale_shift)), int(round(20 * scale_shift)))
        drawBorderAndCornersGL(appShift, 480 * scale_shift, 20 * scale_shift, scale_shift)
        ...
        for i in range(total_segments):
            ...
            if is_flashing:
                col = [1.0, 0.0, 0.4, 1.0] if (int(rpms * 0.1) % 2 == 0) else [0.0, 0.96, 1.0, 1.0]
            elif i < active_segments:
                if i < total_segments * 0.6:
                    col = [0.0, 0.96, 1.0, 0.9] # Neon Cyan
                elif i < total_segments * 0.8:
                    col = [0.75, 0.0, 1.0, 0.9] # Magenta / Violet
                else:
                    col = [1.0, 0.0, 0.4, 0.9] # Hot Pink

            ac.glColor4f(col[0], col[1], col[2], col[3])
            ac.glQuad(int(x), int(round(4 * scale_shift)), int(segment_width), int(12 * scale_shift))
    except Exception as e:
        log_error("drawShiftGL failed:\n" + traceback.format_exc())

def drawTiresGL(deltaT):
    global tireTemps, scale_tires, show_chassis, show_tire_bars, brakeTemps, appTires
    try:
        drawBorderAndCornersGL(appTires, 460 * scale_tires, 140 * scale_tires, scale_tires)
        # Tires (FL, FR, RL, RR)
        # FL
        col = getTireColor(tireTemps[0])
        ac.glColor4f(col[0], col[1], col[2], col[3])
        ac.glQuad(int(round(145 * scale_tires)), int(round(15 * scale_tires)), int(round(18 * scale_tires)), int(round(34 * scale_tires)))

        # FR
        col = getTireColor(tireTemps[1])
        ac.glColor4f(col[0], col[1], col[2], col[3])
        ac.glQuad(int(round(297 * scale_tires)), int(round(15 * scale_tires)), int(round(18 * scale_tires)), int(round(34 * scale_tires)))

        # RL
        col = getTireColor(tireTemps[2])
        ac.glColor4f(col[0], col[1], col[2], col[3])
        ac.glQuad(int(round(145 * scale_tires)), int(round(85 * scale_tires)), int(round(18 * scale_tires)), int(round(34 * scale_tires)))

        # RR
        col = getTireColor(tireTemps[3])
        ac.glColor4f(col[0], col[1], col[2], col[3])
        ac.glQuad(int(round(297 * scale_tires)), int(round(85 * scale_tires)), int(round(18 * scale_tires)), int(round(34 * scale_tires)))

        if show_tire_bars:
            # FL Bar
            ac.glColor4f(0.05, 0.05, 0.05, 0.9)
            ac.glQuad(int(round(145 * scale_tires)), int(round(52 * scale_tires)), int(round(25 * scale_tires)), int(round(5 * scale_tires)))
            col = getTireColor(tireTemps[0])
            ac.glColor4f(col[0], col[1], col[2], col[3])
            pct = max(0.0, min(1.0, (float(tireTemps[0]) - 40.0) / 80.0))
            ac.glQuad(int(round(145 * scale_tires)), int(round(52 * scale_tires)), int(round(25 * pct * scale_tires)), int(round(5 * scale_tires)))

            # FR Bar
            ac.glColor4f(0.05, 0.05, 0.05, 0.9)
            ac.glQuad(int(round(272 * scale_tires)), int(round(52 * scale_tires)), int(round(25 * scale_tires)), int(round(5 * scale_tires)))
            col = getTireColor(tireTemps[1])
            ac.glColor4f(col[0], col[1], col[2], col[3])
            pct = max(0.0, min(1.0, (float(tireTemps[1]) - 40.0) / 80.0))
            ac.glQuad(int(round(272 * scale_tires)), int(round(52 * scale_tires)), int(round(25 * pct * scale_tires)), int(round(5 * scale_tires)))

            # RL Bar
            ac.glColor4f(0.05, 0.05, 0.05, 0.9)
            ac.glQuad(int(round(145 * scale_tires)), int(round(122 * scale_tires)), int(round(25 * scale_tires)), int(round(5 * scale_tires)))
            col = getTireColor(tireTemps[2])
            ac.glColor4f(col[0], col[1], col[2], col[3])
            pct = max(0.0, min(1.0, (float(tireTemps[2]) - 40.0) / 80.0))
            ac.glQuad(int(round(145 * scale_tires)), int(round(122 * scale_tires)), int(round(25 * pct * scale_tires)), int(round(5 * scale_tires)))

            # RR Bar
            ac.glColor4f(0.05, 0.05, 0.05, 0.9)
            ac.glQuad(int(round(272 * scale_tires)), int(round(122 * scale_tires)), int(round(25 * scale_tires)), int(round(5 * scale_tires)))
            col = getTireColor(tireTemps[3])
            ac.glColor4f(col[0], col[1], col[2], col[3])
            pct = max(0.0, min(1.0, (float(tireTemps[3]) - 40.0) / 80.0))
            ac.glQuad(int(round(272 * scale_tires)), int(round(122 * scale_tires)), int(round(25 * pct * scale_tires)), int(round(5 * scale_tires)))

        # Brake Indicator Bars
        ac.glColor4f(1.0, 0.2, 0.2, 0.8)
        bf_pct = max(0.0, min(1.0, float(brakeTemps[0]) / 800.0))
        ac.glQuad(int(round(215 * scale_tires)), int(round(52 * scale_tires)), int(round(30 * bf_pct * scale_tires)), int(round(5 * scale_tires)))
        br_pct = max(0.0, min(1.0, float(brakeTemps[2]) / 800.0))
        ac.glQuad(int(round(215 * scale_tires)), int(round(108 * scale_tires)), int(round(30 * br_pct * scale_tires)), int(round(5 * scale_tires)))
    except Exception as e:
        log_error("drawTiresGL failed:\n" + traceback.format_exc())

def drawSpeedGL(deltaT):
    global scale_speed, speed, text_color_idx, appSpeed
    try:
        drawBorderAndCornersGL(appSpeed, 380 * scale_speed, 220 * scale_speed, scale_speed)
...
def drawPedalsGL(deltaT):
    ...
    try:
        ...
        ac.glQuad(int(round(55 * scale_pedals)), int(round(40 * scale_pedals)), int(round(120 * scale_pedals)), int(round(8 * scale_pedals)))
        ac.glQuad(int(round(55 * scale_pedals)), int(round(62 * scale_pedals)), int(round(120 * scale_pedals)), int(round(8 * scale_pedals)))

        # Fill: Clutch
        ac.glColor4f(0.0, 0.5, 1.0, 0.9)
        ac.glQuad(int(round(55 * scale_pedals)), int(round(18 * scale_pedals)), int(round(120 * clutchInput * scale_pedals)), int(round(8 * scale_pedals)))

        # Fill: Brake
        ac.glColor4f(1.0, 0.2, 0.2, 0.9)
        ac.glQuad(int(round(55 * scale_pedals)), int(round(40 * scale_pedals)), int(round(120 * brakeInput * scale_pedals)), int(round(8 * scale_pedals)))
...
def drawKersGL(deltaT):
    global scale_kers, kersCharge, tyreWear, appKers
    try:
        drawBorderAndCornersGL(appKers, 240 * scale_kers, 55 * scale_kers, scale_kers)
        # Kers Inner Bar
        ac.glColor4f(0.05, 0.05, 0.05, 0.9)
        ac.glQuad(int(round(65 * scale_kers)), int(round(9 * scale_kers)), int(round(15 * scale_kers)), int(round(37 * scale_kers)))
        ac.glColor4f(0.0, 0.94, 1.0, 0.9)
        ac.glQuad(int(round(65 * scale_kers)), int(round(9 * scale_kers + (37 * (1.0 - kersCharge)) * scale_kers)), int(round(15 * scale_kers)), int(round(37 * kersCharge * scale_kers)))

        # Wear Inner Bar
        ac.glColor4f(0.05, 0.05, 0.05, 0.9)
        ac.glQuad(int(round(185 * scale_kers)), int(round(9 * scale_kers)), int(round(15 * scale_kers)), int(round(37 * scale_kers)))
        ac.glColor4f(0.6, 0.6, 0.6, 0.9)
        wear_left = max(0.0, min(1.0, 1.0 - tyreWear))
        ac.glQuad(int(round(185 * scale_kers)), int(round(9 * scale_kers + (37 * (1.0 - wear_left)) * scale_kers)), int(round(15 * scale_kers)), int(round(37 * wear_left * scale_kers)))
    except Exception as e:
        log_error("drawKersGL failed:\n" + traceback.format_exc())

def drawTimesGL(deltaT):
    global scale_times, appTimes
    try:
        drawBorderAndCornersGL(appTimes, 500 * scale_times, 220 * scale_times, scale_times)
    except:
        pass

def drawFuelGL(deltaT):
    global scale_fuel, appFuel
    try:
        drawBorderAndCornersGL(appFuel, 500 * scale_fuel, 220 * scale_fuel, scale_fuel)
    except:
        pass

def drawGearGL(deltaT):
    global scale_gear, appGear
    try:
        drawBorderAndCornersGL(appGear, 380 * scale_gear, 220 * scale_gear, scale_gear)
    except:
        pass

def drawPerfGL(deltaT):
    global scale_perf, appPerf
    try:
        drawBorderAndCornersGL(appPerf, 260 * scale_perf, 80 * scale_perf, scale_perf)
    except:
        pass

def drawDamageGL(deltaT):
    global scale_damage, appDamage
    try:
        drawBorderAndCornersGL(appDamage, 260 * scale_damage, 80 * scale_damage, scale_damage)
    except:
        pass

def drawTrackGL(deltaT):
    global scale_track, appTrack
    try:
        drawBorderAndCornersGL(appTrack, 260 * scale_track, 80 * scale_track, scale_track)
    except:
        pass

def drawDebugGL(deltaT):
    global scale_debug, appDebug
    try:
        drawBorderAndCornersGL(appDebug, 620 * scale_debug, 220 * scale_debug, scale_debug)
    except:
        pass
...
def acUpdate(deltaT):
    ...
    if simInfo is not None:
        ...
        try:
            ...
            ac.setText(lblBrakeF, "{0:.0f}°C".format(brakeTemps[0]))
            ac.setText(lblBrakeR, "{0:.0f}°C".format(brakeTemps[2]))
        except Exception as e:
        ...
        try:
            ...
            ac.setText(lblPerfTurbo, "Turbo: {0:.1f}".format(turbo))
            if drs_enabled:
        ...
        try:
            ...
            ac.setText(lblTrackWind, "Wind: {0:.1f}".format(wind_s))
        except Exception as e:
    ...
    try:
        ...
        ac.setText(lblTimesBest, "B: " + formatTime(best_lap))
        ac.setText(lblTimesLast, "L: " + formatTime(last_lap))
    ...
    if simInfo is not None:
        try:
            ...
            if fuel_per_lap_avg > 0.0:
                ac.setText(lblFuelCons, "Cons: {0:.2f}".format(fuel_per_lap_avg))
                ac.setText(lblFuelEst, "Est: {0:.1f}".format(current_fuel / fuel_per_lap_avg))
            else:
                ac.setText(lblFuelCons, "Cons: --")
                ac.setText(lblFuelEst, "Est: --")
